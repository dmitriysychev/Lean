    ...
    public class EquityFillModel : FillModel
    {
        ...
        private decimal GetBidPrice(Security asset, out DateTime endTime)
        {
            ...
            if (isTickSubscribed)
            {
                ...
                var quote = ticks.LastOrDefault(x => x.TickType == TickType.Quote && x.BidSize != 0);
                if (quote != null)
                {
                    endTime = quote.EndTime;
            ...
            if (isTickSubscribed)
            {
                var trade = ticks.LastOrDefault(x => x.TickType == TickType.Trade && x.BidSize != 0);
                if (trade != null)
                {
                    endTime = trade.EndTime;
                    return trade.Price;
        ...
        private TradeBar GetLastTradeBar(Security asset)
        {
            ...
            if (subscribedTypes.Contains(typeof(Tick)))
            {
                ...
                var quote = ticks.LastOrDefault(x => x.TickType == TickType.Quote && x.AskSize != 0 && x.BidSize != 0);
                if (quote != null)
                {
